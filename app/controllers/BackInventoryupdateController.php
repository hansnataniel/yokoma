...
class BackInventoryupdateController extends BaseController {
	...
	public function getIndex($id)
	{
		...
		if ($admingroup->stockgoods_r != true)
		{
        ...
        return View::make('back.inventory_update.index', $data);
	}

	/* Create a new resource*/
	public function getCreate($branch_id)
	{
		$setting = Setting::first();
		$data['setting'] = $setting;
		...
		$admingroup = Admingroup::find(Auth::admin()->get()->admingroup_id);
		if ($admingroup->stockgoods_c != true)
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/inventory-update')->with('error-message', "Sorry you don't have any priviledge to access this page.");
		}
		...
		if($branch_id != null)
		{
	        ...
	        return View::make('back.inventory_update.create', $data);
		}
		else
		{
	...
	public function getAjaxBlurItem($branch_id)
	{
		...
		return View::make('back.inventory_update.ajax_blur_item', $data);
	}
	...
	public function postAddItem()
	{
		...
		return View::make('back.inventory_update.ajax_items', $data);
	}
	...
	public function getAjaxBlurUpdateItem($item_id, $branch_id)
	{
		...
		return View::make('back.inventory_update.ajax_blur_update_item', $data);
	}

	public function postCreate($branch_id)
	{
		$setting = Setting::first();
		$data['setting'] = $setting;
		...
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/create/' . $branch_id)->with('error-message', "No items you entered.");
		}

		$validator = Validator::make($inputs, $rules);
		if ($validator->passes())
		{
			DB::transaction(function() use ($setting, $branch_id){
				global $stockgood;
				$stockgood = new Stockgood;
				$stockgood->branch_id = htmlspecialchars(Input::get('branch_id'));
				...
				foreach ($items as $item) 
				{
					...
					$inventory_good->save();
					update_inventory($inventory_good->product_id, $inventory_good->branch_id, $inventory_good->date, $inventory_good->id);
				}
			...
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/index/' . $stockgood->branch_id )->with('success-message', "Penyesuain Stock has been Created.");
		}
		else
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/create/' . $branch_id)->withInput()->withErrors($validator);
		}
	}
	...
	public function getView($id)
	{
		...
		{
			$data['stockgood'] = $stockgood;
	        return View::make('back.inventory_update.view', $data);
		}
		else
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/inventory-update')->with('error-message', 'Can not find any stockgood with ID ' . $id);
		}
	}
}
