...
class BackLaporanaccumatiController extends BaseController {
	...
	public function getIndex()
	{
        ...
        return View::make('back.laporan_accu_mati.index', $data);
	}
	...
	public function getReport()
	{
		...
		if($group_by == 'Per Product')
		{
	        ...
	        return View::make('back.laporan_accu_mati.per_product', $data);
		}
		else
		{
			$sale_customers[] = 0; 
	        ...
	        return View::make('back.laporan_accu_mati.per_customer', $data);
		}
		...
		if($group_by == 'per-product')
		{
			...
			Excel::create('Laporan penjualan (produk) ' . date('d-m-Y', strtotime($start_date)) . ' s/d ' . date('d-m-Y', strtotime($end_date)), function($excel) use($data) {
			    ...
			    $excel->sheet('Laporan penjualan', function($sheet) use($data) {
			        ...
			        $sheet->loadView('back.laporan_accu_mati.excel_per_product', $data);
		...
		else
		{
			...
			Excel::create('Laporan penjualan (customer) ' . date('d-m-Y', strtotime($start_date)) . ' s/d ' . date('d-m-Y', strtotime($end_date)), function($excel) use($data) {
			    ...
			    $excel->sheet('Laporan penjualan', function($sheet) use($data) {
			        ...
			        $sheet->loadView('back.laporan_accu_mati.excel_per_customer', $data);

			    });

			})->download('xls');

	        return View::make('back.laporan_accu_mati.excel_per_customer', $data);
		}
	...
	public function getPdf($branch_id, $start_date, $end_date, $group_by)
	{
		...
		if($group_by == 'per-product')
		{
			...
			$html = \View::make('back.laporan_accu_mati.pdf_per_product', $data);
		...
		else
		{
			...
			$html = \View::make('back.laporan_accu_mati.pdf_per_customer', $data);
			$pdf = PDF::loadHTML($html);
			return $pdf->stream();
		}
