...
class BackSalesreturnController extends BaseController {
	...
	public function getIndex()
	{
		...
		if ($admingroup->salesreturn_r != true)
		{
        ...
        return View::make('back.sales_return.index', $data);
	}
	...
	public function getCreate()
	{
		...
		$admingroup = Admingroup::find(Auth::admin()->get()->admingroup_id);
		if ($admingroup->salesreturn_c != true)
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return')->with('error-message', "Sorry you don't have any priviledge to access this page.");
		}
        ...
        return View::make('back.sales_return.create', $data);
	}
	...
	public function getAjaxBranch($branch_id)
	{
		...
		return View::make('back.sales_return.ajax_branch', $data);
	}
	...
	public function getAjaxSales($sales_id)
	{
		...
		return View::make('back.sales_return.ajax_sales', $data);
	}
	...
	public function getAjaxProduct($branch_id, $product_id)
	{
		...
		$inventory = Inventorygood::where('branch_id', '=', $branch_id)->where('product_id', '=', $product_id)->orderBy('date', 'desc')->orderBy('id', 'desc')->first();
		...
		return View::make('back.sales_return.ajax_product', $data);
	}
	...
	public function getAjaxBlurItem($saledetail_id)
	{
		...
		return View::make('back.sales_return.ajax_blur_item', $data);
	}
	...
	public function postAddItem()
	{
		...
		return View::make('back.sales_return.ajax_items', $data);
	}
	...
	public function getAjaxBlurUpdateItem($item_id)
	{
		...
		return View::make('back.sales_return.ajax_blur_update_item', $data);
	}
	...
	public function postCreate()
	{
		...
		{
			$salesreturn = new Salesreturn;
			$salesreturn->branch_id = htmlspecialchars(Input::get('branch_id'));
			$salesreturn->sale_id = htmlspecialchars(Input::get('sale_id'));
			$salesreturn->user_id = 0;
			...
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return')->with('success-message', "Sale Return <strong>$salesreturn->no_invoice</strong> has been Created.");
		}
		else
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return/create')->withInput()->withErrors($validator);
		}
	}
	...
	public function getView($id)
	{
		...
		if ($salesreturn != null)
		{
	        ...
	        return View::make('back.sales_return.view', $data);
		}
		else
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return')->with('error-message', 'Can not find any sale return with ID ' . $id);
		}
	}
	...
	public function getEdit($id)
	{
		...
		$admingroup = Admingroup::find(Auth::admin()->get()->admingroup_id);
		if ($admingroup->salesreturn_u != true)
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return')->with('error-message', "Sorry you don't have any priviledge to access this page.");
		}
		...
		if ($salesreturn != null)
		{
	        ...
	        return View::make('back.sales_return.edit', $data);
		}
		else
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return')->with('error-message', 'Can not find any sale return with ID ' . $id);
	...
	public function putEdit($id)
	{
		...
		{
			$salesreturn = Salesreturn::find($id);
			$old_price_total = $salesreturn->price_total;
			$salesreturn->date = htmlspecialchars(Input::get('date'));
			$salesreturn->save();
			...
			$sale = Sale::find($salesreturn->sale_id);
			$sale->owed = ($sale->owed - $old_price_total) + $salesreturn->price_total;
			if($sale->paid == $sale->owed)
			{
				$sale->status = 'Paid';
			}
			...
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return')->with('success-message', "Sale Return <strong>$salesreturn->no_invoice</strong> has been Updated.");
		}
		else
		{
			return Redirect::to(Crypt::decrypt($setting->admin_url) . '/sales-return/edit/' . $id)->withInput()->withErrors($validator);
		}
	}
	...
	public function getPrintInvoice($id)
	{
		...
		if ($salesreturn != null)
		{
	        ...
	        return View::make('back.sales_return.invoice', $data);
		}
		else
		{
