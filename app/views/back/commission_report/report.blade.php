...
<body>
	...
	<div class='content'>
		...
		@endforeach
		<a target="_blank" href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/commission-report/pdf/' . $salesman->id . '/' . $branch->id . '/' . $start_date . '/' . $end_date)}}">
			{{Form::button('Print', array('class'=>'button-print print'))}}
		</a>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/commission-report/excel/' . $salesman->id . '/' . $branch->id . '/' . $start_date . '/' . $end_date)}}">
