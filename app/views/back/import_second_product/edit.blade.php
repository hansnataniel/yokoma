	...
	<script>
		$(function(){
			...
			$('.branch-id').live('change', function(){
	            ...
	            $.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/ajax-branch')}}/"+selected,
			...
			$('.add-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/ajax-blur-item')}}/" + branchId,
	                success: function(msg){
			...
			$('.ajax-form-item').live('submit', function(e){
				...
				$.ajax({
					type: "POST",
					data: submitData,
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/add-item')}}",
					success:function(msg) {
			...
			$('.delete-item').live('click', function(){
				...
				$.ajax({
					type: "GET",
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/ajax-update-item')}}/" + itemId + '/' + price + '/' + qty ,
					success:function(msg) {
						$('.hasil-ajax-item').html(msg);
						$('.blur-loader').hide();
					}
			...
			$('.update-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/ajax-blur-update-item')}}/" + itemId + '/' + branchId,
	                success: function(msg){
	                    $('#blur-ajax-item').html(msg);
						$('#blur-ajax-item').show();
	                }
	...
	{{Form::model($importsecondproduct, array('url' => URL::current(), 'method' => 'PUT', 'files' => true))}}
		<section id="edit-container">
			...
			<div class="edit-group">
				...
				--><div class="edit-right">
					{{Form::text('branch_name', $importsecondproduct->branch->name, array('class'=>'medium-text readonly', 'readonly'))}}
					{{Form::hidden('branch_id', $importsecondproduct->branch_id, array('class'=>'medium-text readonly branch-id', 'readonly'))}}
					<span class="required-tx">
						*Readonly
					</span>
				</div>
