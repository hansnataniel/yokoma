	...
	<script>
		...
		$(document).ready(function(){
			$('.select-branch').live('change', function(){
				...
				window.location.replace("{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/index')}}/" + branchId);
			});
		});
	</script>
@stop
...
@section('search')
	{{Form::open(array('URL' => URL::current(), 'method' => 'GET'))}}
		<div class="search-group">
			...
			<div class='search-input'>
				{{Form::text('src_date', '', array('class'=>'search-text datetimepicker', 'placeholder'=>'Date', 'readonly'))}}
			</div>
			<div class='search-input'>
...
@section('content')
	<section id="index-container">
		<header id="index-header">
			<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/create')}}" class="index-addnew">
					...
					<td class="icon">
						<div class="index-icon">
							...
							<div class="index-sub-icon">
								<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/view/' . $importsecondproduct->id)}}"><div class="icon-sub">{{HTML::image('img/admin/view.png')}} <span>View</span></div></a>
								<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/edit/' . $importsecondproduct->id)}}"><div class="icon-sub">{{HTML::image('img/admin/edit.png')}} <span>Edit</span></div></a>
								...
								<section class="blur">
									<div class="blur-question">
										...
										<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/import-second-product/delete/' . $importsecondproduct->id . '?_token=' . Session::token())}}">
