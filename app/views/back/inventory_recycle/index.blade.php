...
@section('title')
	Inventory Accu Mati Management
@stop
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('.select-branch').live('change', function(){
				...
				window.location.replace("{{URL::to(Crypt::decrypt($setting->admin_url) . '/inventory-recycle/index')}}/" + branchId);
			});
		});
	</script>
...
@section('help')
	<ul style="padding-left: 18px;">
		<li>Disini anda dapat melihat sekilas data dari inventory Inventory Accu Mati.</li>
	</ul>
@stop
	...
	<section id="index-container">
		...
		<table id="index-table" style="border-spacing: 0px;">
			...
			@foreach ($products as $product)
				<?php 
					$counter++; 
					$inventory = Inventorysecond::where('branch_id', '=', $branch->id)->where('product_id', '=', $product->id)->orderBy('date', 'desc')->orderBy('id', 'desc')->first();
					if($inventory != null)
					{
