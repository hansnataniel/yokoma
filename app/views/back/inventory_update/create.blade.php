	...
	<script>
		$(function(){
			...
			$('.add-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/ajax-blur-item')}}/" + branchId,
			...
			$('.ajax-form-item').live('submit', function(e){
				...
				$.ajax({
					type: "POST",
					data: submitData,
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/add-item')}}",
					success:function(msg) {
			...
			$('.delete-item').live('click', function(){
				...
				$.ajax({
					type: "GET",
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/ajax-update-item')}}/" + itemId + '/' + price + '/' + qty ,
					success:function(msg) {
						$('.hasil-ajax-item').html(msg);
						$('.blur-loader').hide();
					}
			...
			$('.update-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/ajax-blur-update-item')}}/" + itemId + '/' + branchId,
	                success: function(msg){
	                    $('#blur-ajax-item').html(msg);
						$('#blur-ajax-item').show();
	                }
			...
			$('.ajax-form-update-item').live('submit', function(e){
				...
				$.ajax({
					type: "GET",
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/inventory-update/ajax-update-item')}}/" + itemId + '/' + type + '/' + qty ,
					success:function(msg) {
						$('.hasil-ajax-item').html(msg);
						$('#blur-ajax-item').hide();
					}
