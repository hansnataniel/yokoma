...
<body>
	...
	<div class='content'>
		...
		<a target="_blank" href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/laporan-accu-mati/pdf/' . $branch->id . '/' . $start_date . '/' . $end_date . '/per-product')}}">
			{{Form::button('Print', array('class'=>'button-print print'))}}
		</a>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/laporan-accu-mati/excel/' . $branch->id . '/' . $start_date . '/' . $end_date . '/per-product')}}">
