		...
		<table border="0" style="border-spacing: 0;">
			...
			@if(count($payments) != 0)
				...
				<tr class="item-product">
					<td></td>
					<td colspan="4" style="text-align: right;">Total Semua Penerimaan Piutang</td>
					<td style="text-align: right;">Rp. {{digitGroup($price_total)}}</td>
					<td></td>
				</tr>
			@endif
		</table>
		<a target="_blank" href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/laporan-penerimaan-piutang/pdf/' . $branch->id . '/' . $start_date . '/' . $end_date)}}">
			{{Form::button('Print', array('class'=>'button-print print'))}}
		</a>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/laporan-penerimaan-piutang/excel/' . $branch->id . '/' . $start_date . '/' . $end_date)}}">
