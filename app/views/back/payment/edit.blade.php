	...
	<script>
		$(function(){
			...
			$('.update-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/payment/ajax-blur-update-item2')}}/" + itemId,
	                success: function(msg){
	                    $('#blur-ajax-item').html(msg);
						$('#blur-ajax-item').show();
	                }
...
@section('page_title')
	<a href="{{URL::to(Session::get('last_url'))}}">{{HTML::image('img/admin/back.png', '', array('class'=>'image-img'))}}</a> Penerimaan Piutang Edit
@stop
	...
	{{Form::model($payment, array('url' => URL::current(), 'method' => 'PUT', 'files' => true, 'class'=>'form-crate create-submit'))}}
		<section id="edit-container">
			...
			<div class="edit-group">
				...
				--><div class="edit-right">
					{{Form::text('branch', $payment->branch->name, array('class'=>'medium-text readonly', 'readonly'))}}
					<span class="required-tx">
						*Readonly
					</span>
				</div>
