	...
	<script>
		$(function(){
			...
			$('.branch-id').live('change', function(){
				$('.blur-loader').show();
				var selected = "{{$productrepair->branch_id}}";


	            $.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/product-repair/ajax-branch')}}/"+selected,
			...
			$('.ajax-product').live('change', function(){
				var branchId = "{{$productrepair->branch_id}}";
	            ...
	            $.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/product-repair/ajax-product')}}/" + branchId + '/' + productId,
	                success: function(msg){
			...
			$('.add-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/product-repair/ajax-blur-item')}}/" + branchId,
	                success: function(msg){
			...
			$('.ajax-form-item').live('submit', function(e){
				...
				$.ajax({
					type: "POST",
					data: submitData,
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/product-repair/add-item')}}",
					success:function(msg) {
			...
			$('.delete-item').live('click', function(){
				...
				$.ajax({
					type: "GET",
					url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/product-repair/ajax-update-item')}}/" + itemId + '/' + price + '/' + qty ,
					success:function(msg) {
						$('.hasil-ajax-item').html(msg);
						$('.blur-loader').hide();
					}
			...
			$('.update-item').live('click', function(){
				...
				$.ajax({
	                type: "GET",
	                url: "{{URL::to(Crypt::decrypt($setting->admin_url) . '/product-repair/ajax-blur-update-item')}}/" + itemId + '/' + branchId,
	                success: function(msg){
	                    $('#blur-ajax-item').html(msg);
						$('#blur-ajax-item').show();
	                }
...
@section('page_title')
	<a href="{{URL::to(Session::get('last_url'))}}">{{HTML::image('img/admin/back.png', '', array('class'=>'image-img'))}}</a> Klaim Item Edit
@stop
...
@section('content')
	{{Form::model($productrepair, array('url' => URL::current(), 'method' => 'PUT', 'files' => true))}}
		<section id="edit-container">
			<div class="edit-group">
				<div class="edit-left">
					{{Form::label('no_invoice', 'Form No')}}
				</div><!--
				--><div class="edit-right">
					{{Form::text('no_invoice', $productrepair->no_invoice, array('class'=>'medium-text readonly', 'readonly'))}}
			...
			<div class="edit-group">
				...
				--><div class="edit-right">
					{{Form::text('branch_name', $productrepair->branch->name, array('class'=>'medium-text readonly', 'readonly'))}}
					{{Form::hidden('branch_id', $productrepair->branch_id, array('class'=>'medium-text readonly branch-id', 'readonly'))}}
					<span class="required-tx">
						*Readonly
					</span>
				</div>
			...
			<div class="edit-group">
				...
				--><div class="edit-right">
					{{Form::text('date', $productrepair->date, array('class'=>'medium-text datetimepicker', 'required', 'readonly'))}}
					<span class="required-tx">
						*Required
					</span>
				</div>
