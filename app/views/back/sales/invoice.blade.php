<html style="margin: 0; padding: 0;">
	...
	<body style="margin: 0; width: 210mm; height: 148mm; border: solid 1px #000; padding: 20px 0; font-family: arial; font-size 14px;">
										...
										@if($saledetail->type == 'Sales')
											...
											<tr>
												...
												<td style="padding: 3px 10px;line-height: 14px;font-size: 14px;text-align: right;">
													Rp. {{digitGroup($saledetail->price)}}
												</td>
												<td style="padding: 3px 10px;line-height: 14px;font-size: 14px;text-align: center;">
												...
												<td style="padding: 3px 10px;line-height: 14px;font-size: 14px;text-align: right;">
													Rp. {{digitGroup($saledetail->subtotal)}}
												</td>
											</tr>
										@endif
									@endforeach
									...
									@if(count($pembulatans) != 0)
										...
										<tr>
											...
											<td style="padding: 2px 10px; text-align: right;border-top: solid 1px #000; font-size: 14px;">
			                                    Rp. {{digitGroup($total_pembulatan)}}
											</td>
										</tr>										
										<tr>
											<td colspan="5" style="padding: 2px 10px; text-align: right; font-size: 14px;">
												Total
											</td>
											<td style="padding: 2px 10px; text-align: right; font-size: 14px;">
		...
		<div style="margin-left: 20px; margin-bottom: 5px; font-size: 14px; font-weight: bold;">
			Terbilang: <i>{{terbilang($sale->paid)}} rupiah</i>
		</div>

		<div style="display: inline-block; vertical-align: top; text-align: center; margin-left: 20px; margin-top: 25px; font-size: 14px;">
		...
		<div style="display: inline-block;vertical-align: top;margin-left: 20px;margin-top: 25px;font-size: 14px;width: 505px;text-align: left;">
			Keterangan <br>
			@if($sale->keterangan != null)
				<i style="display: block;padding-top: 10px;font-size: 12px;">({{$sale->keterangan}})</i>
			@endif
		</div>
		
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/sales/pdf/' . $sale->id)}}" target="_blank">
			{{Form::button('Print', array('class'=>'print'))}}
		</a>
	</body>
</html>