...
@section('page_title')
	<a href="{{URL::to(Session::get('last_url'))}}">{{HTML::image('img/admin/back.png', '', array('class'=>'image-img'))}}</a> Cancel Sales
@stop
...
@section('content')
	{{Form::model($sale, array('url' => URL::current(), 'method' => 'POST', 'files' => true))}}
		<section id="edit-container">
			...
			<div class="edit-group">
				<div class="edit-left">
					{{Form::label('cancel_karena', 'Cancel Karena')}}
				</div><!--
				--><div class="edit-right">
					{{Form::textarea('cancel_karena', null, array('class'=>'medium-textarea', 'required'))}}
					<span class="required-tx">
						*Required
					</span>
				</div>
