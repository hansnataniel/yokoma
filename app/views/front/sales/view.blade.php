			...
			@if($sale->status == 'Waiting for Payment')
				@if($sale->is_editable == 1)
					...
					@if($requestupdate == null)
						<a href="{{URL::to('sales/update-sales/' . $sale->id)}}"><div class="icon-sub" style="width: 120px;">{{HTML::image('img/admin/edit.png')}} <span>Request Update</span></div></a>
					@endif
				@endif
			@endif
			<div id="view-date">
			...
			<table id="index-table" style="border-spacing: 0px;width: 60%;">
				...
				@foreach($pembulatans as $pembulatan)
					<tr class='index-tr'>
						...
						<td style="text-align: right;">Rp. {{digitGroup($pembulatan->price)}}</td>
					</tr>
					<?php $total_pembulatan = $total_pembulatan + $pembulatan->price; ?>
				@endforeach
				<tr class='index-tr'>
					<td></td>
					<td><strong>Total Pembulatan:</strong></td>
					<td style="text-align: right;">
				...
				<tr class='index-tr'>
					...
					<td><strong>Total Owed:</strong></td>
					<td style="text-align: right;">
						<strong>Rp. {{digitGroup($sale->paid - $total + $total_pembulatan)}}</strong>
					</td>
				</tr>
			</table>
		</article>
