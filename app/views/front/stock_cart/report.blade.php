...
<body>
	...
	<div class='content'>
						...
						<tr class="item">
							...
							<td>
								@if($inventory->status == 'Sale')
									...
									@if($saledetail != null)
										<a href="{{URL::to('sales/view/' . $saledetail->sale->id)}}" target="_blank">
								...
								@elseif($inventory->status == 'Pembelian')
									...
									<a href="{{URL::to('pembelian/view/' . $purchasedetail->purchase->id)}}" target="_blank">
								...
								@elseif(($inventory->status == 'Stock In') OR ($inventory->status == 'Stock Out'))
									...
									<a href="{{URL::to('inventory-update/view/' . $stockgooddetail->stockgood->id)}}" target="_blank">
										{{$inventory->status}}
									</a>
								@else
									{{$inventory->status}}
		...
		@endforeach
		@if($type == 'specific')
			<a target="_blank" href="{{URL::to('stock-cart/pdf/' . $branch->id . '/' . $start_date . '/' . $end_date . '/' . $product->id . '/' . $type)}}">
				{{Form::button('Print', array('class'=>'button-print print'))}}
			</a>
			<a href="{{URL::to('stock-cart/excel/' . $branch->id . '/' . $start_date . '/' . $end_date . '/' . $product->id . '/' . $type)}}">
				{{Form::button('Excel', array('class'=>'button-print excel'))}}</a>
		@elseif($type == 'all')
			<a target="_blank" href="{{URL::to('stock-cart/pdf/' . $branch->id . '/' . $start_date . '/' . $end_date . '/0/' . $type)}}">
				{{Form::button('Print', array('class'=>'button-print print'))}}
			</a>
			<a href="{{URL::to('stock-cart/excel/' . $branch->id . '/' . $start_date . '/' . $end_date . '/0/' . $type)}}">
				{{Form::button('Excel', array('class'=>'button-print excel'))}}</a>
		@else
			<a target="_blank" href="{{URL::to('stock-cart/pdf/' . $branch->id . '/' . $start_date . '/' . $end_date . '/' . $product_name . '/' . $type)}}">
				{{Form::button('Print', array('class'=>'button-print print'))}}
			</a>
			<a href="{{URL::to('stock-cart/excel/' . $branch->id . '/' . $start_date . '/' . $end_date . '/' . $product_name . '/' . $type)}}">
				{{Form::button('Excel', array('class'=>'button-print excel'))}}</a>
		@endif
	</div>
</body>
